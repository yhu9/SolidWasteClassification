        ...
        if(sys.argv[1] == 'train'):
            ...
            with tf.Session() as sess:
                ...
                for epoch in range(constants.CNN_EPOCHS):
                    ...
                    if epoch % 1 == 0:
                        #record summaries
                        summary = tf.summary.merge([acc_sum1,acc_sum2,acc_sum3,acc_sum4,acc_sum5,acc_sum6,allacc_sum,cost_sum1,cost_sum2,cost_sum3,cost_sum4,cost_sum5,cost_sum6,allcost_sum])
                ...
                for i in range(int(constants.IMG_SIZE / 2),int(len(image) - (constants.IMG_SIZE / 2))):
                    for j in range(int(constants.IMG_SIZE / 2),int(len(image[0]) - (constants.IMG_SIZE / 2))):
                        ...
                        if(len(tmp) == constants.BATCH_SIZE or count == ((h - constants.IMG_SIZE) * (w - constants.IMG_SIZE)) - 1):
                            batch = np.array(tmp)
                            rawpredictions = predictions_out.eval({x1:batch, x2:batch, x3:batch, x4:batch, x5:batch, x6:batch})
